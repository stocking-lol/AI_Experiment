...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="pikafish",
        help="rival type: [minmax, alphabeta, pikafish]",
    )
    # 控制piksfish的棋力
    parser.add_argument(
        "--pika_skill", type=int, default=20, help="skill level of pikafish"
    )
    parser.add_argument(
        "--pika_depth", type=int, default=5, help="max depth of pikafish search"
    )
    ...
    background_img = pygame.image.load("./images/bg2.jpg")
    ...
    # 创建AI对象
    if args.ai == "minmax":
        ai = ChessAI(game.computer_team, search_with="minmax")
    elif args.ai == "alphabeta":
        ai = ChessAI(game.computer_team, search_with="alphabeta")
    elif args.ai == "pikafish":
        ai = PikafishEngine(
            game.computer_team, max_depth=args.pika_depth, skill_level=args.pika_skill
        )

    message = ["Game start!!!", "Palyer come!!!", f"AI [{args.ai}] joined!!!"]
    aimess = [
        "What a attack from ai!",
        "Clever move from ai",
        "Ai says u're smart...",
        "Ai do a good job!",
    ]
        ...
        if not game.show_win and game.AI_mode and game.get_player() == ai.team:
            ...
            for event in pygame.event.get():
                ...
                if not game.show_win:
                    ...
                    if clicked_dot:
                        tempstr = chessboard.move_chess(
                            clicked_dot.row, clicked_dot.col
                        )
                        # 清理「点击对象」、「可落子位置对象」
                        ...
                        if chessboard.judge_attack_general(game.get_player()):
                            ...
                            if chessboard.judge_win(game.get_player()):
                                ...
                                message.append("Winner has come!")
                                game.set_win(game.get_player())
                        game.back_button.add_history(
                            chessboard.get_chessboard_str_map()
                        )
                        # 落子之后，交换走棋方
                        game.exchange()
                    ...
                    clicked_chess = Chess.get_clicked_chess(
                        game.get_player(), chessboard
                    )
                    if clicked_chess:
                        # 创建选中棋子对象
                        ...
                        Dot.clean_last_position()
                        # 计算当前被点击的棋子可以落子的位置
                        put_down_chess_pos = chessboard.get_put_down_position(
                            clicked_chess
                        )
                        # 根据当前被点击的棋子创建可以落子的对象
