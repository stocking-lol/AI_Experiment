...
class ChessBoard(object):
    ...
    def printstr(self):
        """打印棋盘字符串"""
        row_idx = 0
        for line in self.get_chessboard_str_map():
            line_str = ""
            for chess in line:
                if chess:
                    line_str += chess + " "
    ...
    def create_chess(self):
        ...
        for row, line in enumerate(self.chessboard_map):
            for col, chess_name in enumerate(line):
                if chess_name:
                    # 将创建的棋子添加到属性map中
                    self.chessboard_map[row][col] = Chess(
                        self.screen, chess_name, row, col
                    )
                else:
        ...
        for row in range(10):
            ...
            for col in range(9):
                if self.chessboard_map[row][col]:
                    line_map.append(
                        self.chessboard_map[row][col].team
                        + "_"
                        + self.chessboard_map[row][col].name
                    )
                else:
                    line_map.append("")
    ...
    def get_put_down_position(self, clicked_chess):
        ...
        if clicked_chess.name == "z":  # 卒
            if team == "r":  # 红方
                ...
                if row + 1 <= 9:  # 只能向下移动
                    if not map_[row + 1][col] or map_[row + 1][col].team != team:
                        put_down_chess_pos.append((row + 1, col))
            # 左右判断
            if (team == "r" and 0 <= row <= 4) or (
                team == "b" and 5 <= row <= 9
            ):  # 左、右一步
                # 左
                if col - 1 >= 0 and (
                    not map_[row][col - 1] or map_[row][col - 1].team != team
                ):
            ...
            # 有4个方向的判断
            if row - 1 >= row_start and (
                not map_[row - 1][col] or map_[row - 1][col].team != team
            ):
                put_down_chess_pos.append((row - 1, col))
            if row + 1 <= row_stop and (
                not map_[row + 1][col] or map_[row + 1][col].team != team
            ):
                put_down_chess_pos.append((row + 1, col))
            if col - 1 >= 3 and (
            ...
            if (
                row - 1 >= row_start
                and col - 1 >= 3
                and (not map_[row - 1][col - 1] or map_[row - 1][col - 1].team != team)
            ):
            ...
            if (
                row - 2 >= row_start and col - 2 >= 0 and not map_[row - 1][col - 1]
            ):  # 左上
                if not map_[row - 2][col - 2] or map_[row - 2][col - 2].team != team:
                    put_down_chess_pos.append((row - 2, col - 2))
            if (
                row - 2 >= row_start and col + 2 <= 8 and not map_[row - 1][col + 1]
            ):  # 右上
                if not map_[row - 2][col + 2] or map_[row - 2][col + 2].team != team:
                    put_down_chess_pos.append((row - 2, col + 2))
            if (
                row + 2 <= row_stop and col - 2 >= 0 and not map_[row + 1][col - 1]
            ):  # 左下
                if not map_[row + 2][col - 2] or map_[row + 2][col - 2].team != team:
                    put_down_chess_pos.append((row + 2, col - 2))
            if (
                row + 2 <= row_stop and col + 2 <= 8 and not map_[row + 1][col + 1]
            ):  # 右下
                if not map_[row + 2][col + 2] or map_[row + 2][col + 2].team != team:
                    put_down_chess_pos.append((row + 2, col + 2))
            ...
            # 上方
            if (
                row - 1 >= 0 and not map_[row - 1][col]
            ):  # 如果当前棋子没有被蹩马腿，那么再对这个方向的2个位置进行判断
                # 左上
                if (
                    row - 2 >= 0
                    and col - 1 >= 0
                    and (
                        not map_[row - 2][col - 1]
                        or map_[row - 2][col - 1].team != team
                    )
                ):
            ...
            # 下方
            if (
                row + 1 <= 9 and not map_[row + 1][col]
            ):  # 如果当前棋子没有被蹩马腿，那么再对这个方向的2个位置进行判断
                # 左下
                if (
                    row + 2 <= 9
            ...
            # 左方
            if (
                col - 1 >= 0 and not map_[row][col - 1]
            ):  # 如果当前棋子没有被蹩马腿，那么再对这个方向的2个位置进行判断
                # 左上2（因为有左上了，暂且称为左上2吧）
                if (
                    row - 1 >= 0
                    and col - 2 >= 0
                    and (
                        not map_[row - 1][col - 2]
                        or map_[row - 1][col - 2].team != team
                    )
                ):
            ...
            # 右方
            if (
                col + 1 <= 8 and not map_[row][col + 1]
            ):  # 如果当前棋子没有被蹩马腿，那么再对这个方向的2个位置进行判断
                # 右上2（因为有右上了，暂且称为右上2吧）
                if (
                    row - 1 >= 0
            ...
            for i in range(1, 9):
                ...
                if direction_left_chess_num >= 0 and col - i >= 0:
                    if not map_[row][col - i] and direction_left_chess_num == 0:
                        ...
                        if (
                            direction_left_chess_num == 2
                            and map_[row][col - i].team != team
                        ):
                            put_down_chess_pos.append((row, col - i))
                            direction_left_chess_num = (
                                -1
                            )  # 让其不能够在下次for循环时再次判断
                # 右边位置没有越界
            ...
            for i in range(1, 10):  # 这样就让i从1开始，而不是从0
                ...
                if direction_up_chess_num >= 0 and row - i >= 0:
                    if not map_[row - i][col] and direction_up_chess_num == 0:
                        ...
                        if (
                            direction_up_chess_num == 2
                            and map_[row - i][col].team != team
                        ):
                            put_down_chess_pos.append((row - i, col))
                            direction_up_chess_num = -1
        ...
        # 剔除哪些被"将军"的位置
        put_down_chess_pos = self.judge_delete_position(
            put_down_chess_pos, clicked_chess
        )
    ...
    def move_chess_silent(self, old_row, old_col, new_row, new_col):
        """
        将棋子移动到指定位置
        """
        # 得到要移动的棋子的位置
    ...
    def judge_j_attack(self, attack_row, attack_col, general_row, general_col):
        ...
        if attack_col == general_col:
            # 在同一列
            min_row, max_row = (
                (attack_row, general_row)
                if attack_row < general_row
                else (general_row, attack_row)
            )
    ...
    def judge_m_attack(self, attack_row, attack_col, general_row, general_col):
        ...
        if attack_row == general_row or attack_col == general_col:
            ...
            if col_length + row_length == 5:
                ...
                if col_length == 1:
                    if (
                        general_row < attack_row
                        and not self.chessboard_map[attack_row - 1][attack_col]
                    ):
                        return True
                    elif (
                        general_row > attack_row
                    ...
                    if (
                        general_col < attack_col
                        and not self.chessboard_map[attack_row][attack_col - 1]
                    ):
                        return True
                    elif (
                        general_col > attack_col
    ...
    def judge_c_and_p_attack(
        self, attack_chess_name, attack_row, attack_col, general_row, general_col
    ):
        """
        判断"车"、"炮"能否攻击到对方"将"
        """
        ...
        if attack_row == general_row:
            # 在同一行
            min_col, max_col = (
                (attack_col, general_col)
                if attack_col < general_col
                else (general_col, attack_col)
            )
            ...
            for col, chess in enumerate(line):
                if chess and chess.team == attack_player:
                    if chess.name == "z":  # 兵
                        ...
                        if self.judge_z_attack(
                            chess.team, chess.row, chess.col, *general_position
                        ):
                            return True
                    elif chess.name == "p":  # 炮
                        if self.judge_c_and_p_attack(
                            chess.name, chess.row, chess.col, *general_position
                        ):
                            return True
                    elif chess.name == "c":  # 车
                        if self.judge_c_and_p_attack(
