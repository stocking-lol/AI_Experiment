...
def atMostOne(literals):
    ...
    return conjoin([~literals[i] | ~literals[j] for i in range(len(literals)) for j in range(i+1, len(literals))])
    "*** END YOUR CODE HERE ***"
...
def pacphysics_axioms(t, all_coords, non_outer_wall_coords):
    ...
    pacphysics_sentences.append(exactlyOne([PropSymbolExpr(direction_str, t) for direction_str in DIRECTIONS]))
    "*** END YOUR CODE HERE ***"
...
def positionLogicPlan(problem):
    ...
    for t in range(50):  
        ...
        KB.extend([pacmanSuccessorStateAxioms(x,y,t+1,walls) for x,y in non_wall_coords])
...
def foodLogicPlan(problem):
    ...
    KB.append(PropSymbolExpr(pacman_str, x0, y0, 0))
    KB.extend([PropSymbolExpr(food_str, x, y, 0) for x,y in food])
    
    for t in range(50):
        print(t)
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, t) for x, y in non_wall_coords]))
        goal_expr = conjoin([~PropSymbolExpr(food_str, x, y, t) for x, y in food])
        model = findModel(conjoin(KB + [goal_expr]))
        if model:
            return extractActionSequence(model, actions)

        KB.extend([PropSymbolExpr(food_str,x,y,t+1) % (PropSymbolExpr(food_str,x,y,t) & ~PropSymbolExpr(pacman_str,x,y,t))
                   for x,y in food])
...
def mapping(problem, agent):
    ...
    for t in range(agent.num_timesteps):
        ...
        for i in known_map:
            ...
            for j in i:
                if j==-1:
                    new_walls_tmp.append(0)
                else:
                    new_walls_tmp.append(j)
            new_walls.append(new_walls_tmp)

        KB.append(allLegalSuccessorAxioms(t + 1, new_walls, non_outer_wall_coords))
    "*** END YOUR CODE HERE ***"
...
def slam(problem, agent):
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            model1 = findModel(conjoin(KB) & ~PropSymbolExpr(wall_str, x, y))
            model2 = findModel(conjoin(KB) & PropSymbolExpr(wall_str, x, y))
            model3 = findModel(conjoin(KB) & ~PropSymbolExpr(pacman_str, x, y, t))
            model4 = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, x, y, t))
        ...
        KB.append(SLAMSuccessorAxioms(t+1, new_walls, non_outer_wall_coords))
